...
export const AccountSection: React.FC<AccountSectionProps> = ({
  ...
  return (
    <div className="space-y-4" id="account-section">
      ...
      <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200/90 shadow-xs relative overflow-hidden space-y-4">
          ...
          <div className="flex items-center gap-3">
            {profile?.photoURL || user?.photoURL ? (
              <img
                ...
                alt="Avatar"
                className="w-12 h-12 rounded-2xl object-cover ring-2 ring-indigo-500/20 shadow-2xs"
                referrerPolicy="no-referrer"
              />
            ) : (
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 via-indigo-600 to-blue-600 text-white font-extrabold text-base flex items-center justify-center shadow-2xs ring-1 ring-white/20">
                {avatarInitials}
              </div>
            )}
        ...
        <div className="flex flex-wrap gap-2 pt-1 text-[11px] font-semibold">
          <span className="px-2.5 py-1 rounded-xl bg-slate-100 text-slate-700 border border-slate-200/80">
            {profile?.jobTitle || 'Team Member'}
          </span>
          <span className="px-2.5 py-1 rounded-xl bg-indigo-50 text-indigo-700 border border-indigo-200/60">
            {profile?.department || 'Engineering'}
          </span>
          <span className="px-2.5 py-1 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200/60">
          ...
          <form onSubmit={handleSaveProfile} className="space-y-3 pt-3 border-t border-slate-100">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-medium text-slate-900 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 outline-none"
              />
            </div>
        ...
        <div className="space-y-2 pt-2 border-t border-slate-100">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs transition-colors cursor-pointer border border-slate-200/80"
            id="btn-switch-account"
          >
            <Users className="w-4 h-4 text-indigo-600" />
            <span>Switch Account (Google / Email)</span>
          </button>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs transition-colors cursor-pointer border border-rose-200/70"
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200/90 shadow-xs space-y-2.5">
        ...
        <p className="text-[11px] text-slate-600 leading-relaxed">
          Your attendance records, daily timestamps, and custom targets are synced to Firebase Firestore database.
        </p>

        <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/80 text-[10px] text-slate-500 font-mono break-all">
          UID: {user?.uid}
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200/90 shadow-xs space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs transition-colors cursor-pointer border border-slate-200/80"
          >
        ...
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
          ...
          >
            Load Sample Demo Week
          </button>

          <button
            type="button"
            onClick={() => {
              if (window.confirm('Erase all attendance records for this account?')) {
                onClearAll();
              }
            }}
